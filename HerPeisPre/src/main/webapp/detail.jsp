...
<script type="text/javascript">
		...
		//alert(GetQueryString("sex"));
		//alert(GetQueryString("id"));
		//alert(GetQueryString("price"));
		 $.ajax({
	         url: 'detailinfo',
	         type: 'POST',
	         
	         dataType:'json',
	         data: {sid:GetQueryString("id")}
	       })
	       .done(function(data) {
	    	 //接收传来的json
	    	...
	    	$.each(data,function(j,result){
	    		         k=j+1;
	    				item =	"<tr style='text-align:center'><td>"+k+"</td><td>"+result.examname+"</td><td >"+result.price+"元</td></tr>";
...
<div class="col-info">
	  ...
	  <p style="font-weight: bold;">￥<%=request.getParameter("price") %></p>
	  <div class="pre">
	  	<a id="pre" href="preinfo.jsp?itemid=<%=id %>&itemname=<%=name%>&itemsex=<%=sex %>&itemprice=<%=price %>">我要预约</a>
	  ...
	  <dl style="font-family: '微软雅黑'; ">
	  	...
	  	<dd style="float: left;color: #999999">适宜人群:&nbsp&nbsp&nbsp&nbsp</dt>
	  	<dd style="float: left"><%=name %></dd> 
	  	<span style="color: #FFAA38"> (<%=sex %>) </span></br>
	  	<h5  style="color: #FFAA38"> (<%=id %>) </h5>
	  </dl>
	  </div>
